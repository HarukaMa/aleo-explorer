    ...
    for finalize_operation_db_id, type, index in
        ...
        loop
            if type = 'InitializeMapping' then
                select t.mapping_id from finalize_operation_initialize_mapping t where t.id = finalize_operation_db_id into mapping_id;
                return next;
            elsif type = 'InsertKeyValue' then
                select t.mapping_id, t.key_id, t.value_id from finalize_operation_insert_kv t where t.id = finalize_operation_db_id into mapping_id, key_id, value_id;
                return next;
            elsif type = 'UpdateKeyValue' then
                select t.mapping_id, t.key_id, t.value_id from finalize_operation_update_kv t where t.id = finalize_operation_db_id into mapping_id, key_id, value_id;
                return next;
            elsif type = 'RemoveKeyValue' then
                select t.mapping_id from finalize_operation_remove_kv t where t.id = finalize_operation_db_id into mapping_id;
                return next;
            elsif type = 'RemoveMapping' then
                select t.mapping_id from finalize_operation_remove_mapping t where t.id = finalize_operation_db_id into mapping_id;
                return next;
            else
